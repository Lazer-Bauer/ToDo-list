...
todoList.addEventListener("change", (e) => {
  ...
  if (target.matches(`input.is-complete`)) {
    console.log(
      (target.closest(`li.to-do-item`) as HTMLElement).dataset.toDoId
    );
    handleIsCompleteChange(
      Number((target.closest(`li.to-do-item`) as HTMLElement).dataset.toDoId)
    );
  }
});
...
function renderTodoItem({ id, title, isCorrect }: Todo) {
  return `
