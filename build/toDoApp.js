...
const { getToDos, addToDos, removeToDos, changeToDos, removeAll, getToDoById, saveTodo, } = createTodoList();
// render
...
todoList.addEventListener("change", (e) => {
    ...
    if (target.matches(`input.is-complete`)) {
        console.log(target.closest(`li.to-do-item`).dataset.toDoId);
        handleIsCompleteChange(Number(target.closest(`li.to-do-item`).dataset.toDoId));
    }
});
todoList.addEventListener("click", (e) => {
    const target = e.target;
    if (target.closest(`.todo-remove-btn`)) {
        handleRemoveTodo(Number(target.closest(`li.to-do-item`).dataset.toDoId));
    }
});
...
function renderTodoItem({ id, title, isCorrect }) {
  ...
  <li data-to-do-id="${id}" class="to-do-item list-group-item d-flex justify-content-between">
    ...
    <label
      class="form-check-label mx-1 flex-fill  ${isCorrect ? `text-muted text-decoration-line-through` : ""}"
      for="todo-${id}"
    >
      ${title}
    </label>
